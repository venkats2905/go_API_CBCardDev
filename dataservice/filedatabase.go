...
func GetProcessedfilesFromDb(processedfiles []models.ProcessedFile) []models.ProcessedFile {
	...
	rows, err := db.QueryContext(ctx, "SELECT * FROM ORACLEDATABASE.processedfiles")
...
func GetprocessedfilesByfileidFromDb(fileid int) (models.ProcessedFile, error) {
	...
	query := `SELECT * FROM ORACLEDATABASE.processedfiles WHERE Fileid = :1`
...
// 	var processedfile models.ProcessedFile // Change to a single instance of ProcessedFile
// 	// ...

// 	if rows.Next() {
// 		err := rows.Scan(
// 			&processedfile.Fileid, &processedfile.Source_system, &processedfile.Platform,
// 			&processedfile.Isproduction, &processedfile.Header_date, &processedfile.Process_date,
// 			&processedfile.Start_seq, &processedfile.End_seq, &processedfile.File_name,
// 			&processedfile.Mabx_csm_fileid)
// 		if err != nil {
// 			return models.ProcessedFile{}, err // Return an empty ProcessedFile in case of error
// 		}
// 		fmt.Println("Got a particular Processed File from the table successfully")
// 	}

// 	return processedfile, nil // Return a single ProcessedFile, not a slice
// }
...
func PostAddProcessedfileToDb(processedfiles models.ProcessedFile) {
	...
	query := `INSERT INTO ORACLEDATABASE.processedfiles (Fileid, Source_system, Platform,
		Isproduction,Header_date,Process_date, Start_seq, End_seq, File_name, Mabx_csm_fileid) VALUES (:1, :2, :3, :4, :5, :6, :7, :8, :9)`

	fmt.Println("QUERY:", query)

	_, err := db.ExecContext(ctx, query,
		processedfiles.Fileid, processedfiles.Platform, processedfiles.Source_system, processedfiles.Platform,
		processedfiles.Isproduction, processedfiles.Header_date, processedfiles.Process_date,
		processedfiles.Start_seq, processedfiles.End_seq, processedfiles.File_name,
		processedfiles.Mabx_csm_fileid)
	if err != nil {
...
func DeleteProcessedfileFromDb(fileid int) {
	...
	query := fmt.Sprintf("DELETE FROM ORACLEDATABASE.processedfiles WHERE fileid = %d", fileid)
	fmt.Println("Query:", query)
...
func UpdateProcessedfileInDb(processedfiles models.ProcessedFile) {
	...
	query := `UPDATE ORACLEDATABASE.Processedfiles SET Fileid =:1, Source_system = :2, Platform= :3,
	Isproduction = :4,Header_date = :5,Process_date = :6, Start_seq = :7, End_seq = :8, File_name = :9, Mabx_csm_fileid = :10`

	fmt.Println("Query:", query)

	_, err := db.ExecContext(ctx, query,
		processedfiles.Platform, processedfiles.Source_system, processedfiles.Platform,
		processedfiles.Isproduction, processedfiles.Header_date, processedfiles.Process_date,
		processedfiles.Start_seq, processedfiles.End_seq, processedfiles.File_name,
		processedfiles.Mabx_csm_fileid)
	if err != nil {
