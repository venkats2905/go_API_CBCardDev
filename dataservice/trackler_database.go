...
func GetTracking(Trackrecord []models.Trackingsequence_archive, db *sql.DB) []models.Trackingsequence_archive {
	...
	//defer rows.Close()
	for rows.Next() {
		var a models.Trackingsequence_archive
...
func TrackbySID(SID int, db *sql.DB) (models.Trackingsequence_archive, error) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func Postrecord(Trackrecord models.Trackingsequence_archive, db *sql.DB) {
	fmt.Println("\nin Postrecord ", db)
...
func Updaterecord(Trackrecord models.Trackingsequence_archive, db *sql.DB, SID string) {
